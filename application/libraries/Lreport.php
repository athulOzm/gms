...
// Retrieve Single Item Stock Stock Report
    public function stock_report_single_item($product_id = null, $date = null, $links, $per_page, $page) {
        $CI = & get_instance();
        $CI->load->model('Reports');
        $CI->load->library('occational');
...
// Retrieve todays_sales_report
    ...
    //======================= user sales report ================================
    public function user_sales_report($links = null, $per_page = null, $page = null, $from_date, $to_date) {
        $CI = & get_instance();
        $CI->load->model('Reports');
        $CI->load->model('Web_settings');
        $CI->load->library('occational');
        $sales_report = $CI->Reports->user_sales_report($per_page, $page, $from_date, $to_date);
        $sales_amount = 0;
        if (!empty($sales_report)) {
            $i = 0;
            foreach ($sales_report as $k => $v) {
        ...
        $data = array(
            ...
            'sales_report' => $sales_report,
            'company_info' => $company_info,
            'from' => $CI->occational->dateConvertMyformat($from_date),
            'to' => $CI->occational->dateConvertMyformat($to_date),
            'currency' => $currency_details[0]['currency'],
            'position' => $currency_details[0]['currency_position'],
            'links' => $links,
            'software_info' => $currency_details,
    ...
    public function sales_return_data($links, $perpage, $page, $start, $end) {
        ...
        if (!empty($return_list)) {
            foreach ($return_list as $k => $v) {
                $return_list[$k]['final_date'] = $CI->occational->dateConvertMyformat($return_list[$k]['date_return']);
            }
            $i = 0;
            if (!empty($return_list)) {
                foreach ($return_list as $k => $v) {
