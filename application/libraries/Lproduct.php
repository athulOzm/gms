...
//        dd($supplier_selected);
        ...
        $tax_list = $CI->db->select('*')
            ...
            $data['tax_selecete']     = $product_detail[0]['tax'];
            $data['minimum_stock']             = $product_detail[0]['minimum_stock'];
            $data['reorder_level']             = $product_detail[0]['reorder_level'];
            $data['product_location']             = $product_detail[0]['product_location'];
            $data['global_markup']             = $product_detail[0]['global_markup'];
            $data['individual_markup']             = $product_detail[0]['individual_markup'];
            $data['notes']             = $product_detail[0]['notes'];
...
//                $purchaseData[$k]['final_date'] = $CI->occational->dateConvert($purchaseData[$k]['purchase_date']);
                $purchaseData[$k]['final_date'] = $CI->occational->dateConvertMyformat($purchaseData[$k]['purchase_date']);
        ...
        if (!empty($salesData)) {
            foreach ($salesData as $k => $v) {
                $salesData[$k]['final_date'] = $CI->occational->dateConvertMyformat($salesData[$k]['date']);
