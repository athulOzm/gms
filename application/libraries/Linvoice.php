...
class Linvoice {
    ...
    public function invoice_list($links, $perpage, $page, $user_id, $user_type) {
        $CI = & get_instance();
        $CI->load->model('Invoices');
        $CI->load->model('Web_settings');
        $CI->load->library('occational');

        $invoices_list = $CI->Invoices->invoice_list($perpage, $page, $user_id, $user_type);
...
//        dd($data);
    ...
    //Retrieve  Invoice List
    public function recurring_invoice_list($links, $perpage, $page) {
        ...
        $invoices_list = $CI->Invoices->recurring_invoice_list($perpage, $page);
//        echo '<pre>';        print_r($invoices_list);die();
        if (!empty($invoices_list)) {
            foreach ($invoices_list as $k => $v) {
                $invoices_list[$k]['final_date'] = $CI->occational->dateConvert($invoices_list[$k]['date']);
...
//        dd($data);
        $invoiceList = $CI->parser->parse('invoice/recurring_invoice', $data, true);
        return $invoiceList;
    }
