...
//        d($supplier_detail);
        $data = array(
            ...
            'website' => $supplier_detail[0]['website'],
            'notification' => $supplier_detail[0]['notification'],
            'payment_method' => $supplier_detail[0]['payment_method'],
            'order_by' => $supplier_detail[0]['order_by'],
    ...
    public function supplier_detail_data($supplier_id) {
        ...
        if (!empty($purchase_info)) {
            foreach ($purchase_info as $k => $v) {
                $purchase_info[$k]['final_date'] = $CI->occational->dateConvertMyformat($purchase_info[$k]['purchase_date']);
