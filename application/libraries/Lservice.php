...
//            'taxfiled'     => $taxfield,
    ...
    public function service_add_form() {
        ...
        $taxfield = $CI->db->select('tax_name,default_value')
                ...
                ->get()
                ->result_array();
        $get_service_types = $CI->db->select('*')->from('service_type')->where('status', 1)->get()->result();
        $data = array(
...
//                  $i++;
//                }
        $data['servicetype_wise_category'] = $CI->db->select('*')->from('service_category')->where('service_type_id', $service_detail[0]['service_type_id'])
                        ->order_by('service_category_name')->get()->result();
        $data['title'] = display('service_edit');
        $data['service_id'] = $service_detail[0]['service_id'];
        $data['service_type_id'] = $service_detail[0]['service_type_id'];
        $data['service_category_id'] = $service_detail[0]['service_category_id'];
        $data['service_name'] = $service_detail[0]['service_name'];
        $data['service_rate'] = $service_detail[0]['service_rate'];
        $data['standard_timing'] = $service_detail[0]['standard_timing'];
        $data['get_service_types'] = $get_service_types;
