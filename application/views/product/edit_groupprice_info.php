            ...
            <div class="col-sm-12">
                <div class="panel panel-bd lobidrag">
                    ...
                    <div class="panel-body">
                        <?php echo form_open('Cproduct/group_pricing_update', array('class' => 'form-vertical', 'id' => 'insert_menu')) ?>
                        <div class="form-group row">
                            <label for="group_name" class="col-sm-3 text-right"><?php echo display('group_name'); ?> <span class="text-danger"> * </span></label>
                            <div class="col-sm-8">
                                <input type="text" name="group_name" id="group_name" class="form-control" value="<?php echo $edit_groupprice_info[0]->group_name; ?>" placeholder="Enter Group Name">
                            </div>
                        </div>
...
//                                    d($get_groupdetails_info);
                                    $sl = 0;
                                    foreach ($get_groupdetails_info as $single) {
                                        $sl++;
                                        ?>
                                        <tr>
                                            <td>
                                                <select class="form-control product_id common_pdt" id="product_id_<?php echo $sl; ?>" onchange="service_cals(this.value, '<?php echo $sl; ?>')" name="product_id[]">
                                                    <option value=""><?php echo display('select_one'); ?></option>
                                                    <?php foreach ($get_products as $product) { ?>
                                                        <option value='<?php echo $product->product_id; ?>' <?php
                                                        if ($single->product_id == $product->product_id) {
                                                            echo 'selected';
                                                        }
                                                        ?>>
                                            ...
                                            <td>
                                                <input class="form-control text-center" type="number" min="1" onkeyup="quantity_calculate(<?php echo $sl; ?>);" onchange="quantity_calculate(<?php echo $sl; ?>);" value="<?php echo $single->group_product_qty; ?>" name="group_product_qty[]" id="group_product_qty_<?php echo $sl; ?>">
                                            </td>
                                            <td>
                                                <input class="form-control text-right regular_price" type="text" value="<?php echo $single->regular_price; ?>" name="regular_price[]" id="regular_price_<?php echo $sl; ?>" readonly>
                                                <input type='hidden' value="<?php echo $single->group_product_qty*$single->regular_price;?>" class='quantity_regularprice' id='quantity_regularprice_<?php echo $sl; ?>'>
                                            </td>
                                            <td class="test">
                                                <input type="text" name="product_rate[]" onkeyup="groupquantity_calculate(<?php echo $sl; ?>);" value="<?php echo $single->product_rate; ?>" onchange="groupquantity_calculate(<?php echo $sl; ?>);" id="product_rate_<?php echo $sl; ?>" class="form-control text-right productrate" placeholder="0.00" value="" min="0"  required=""/>
                                                <input type='hidden' value="<?php echo $single->group_product_qty*$single->product_rate;?>" class='quantity_productrate' id='quantity_productrate_<?php echo $sl; ?>'>
                                            </td>
    <!--                                            <td class="test">
                                                    <input type="text" name="product_rate[]" id="product_rate_<?php echo $sl; ?>" class="form-control common_pdt_rate total_price text-right" placeholder="0.00" value="<?php echo $single->product_rate; ?>" min="0" required=""/>
                                                </td>-->
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td class="text-right">
                                            <input type="text" id="cumulative_price" class="form-control text-right cumulative_price" value="<?php echo $edit_groupprice_info[0]->cumulative_price; ?>" name="cumulative_price" value="0.00" readonly="readonly" />
                                        </td>
                                       <td>                                            
                                            <b><?php echo display('group_price'); ?>:</b>
                                            <input type="text" value="<?php echo $edit_groupprice_info[0]->groupprice; ?>" id="groupprice" class="form-control text-right groupprice_txtcls" name="groupprice" placeholder="0.00" tabindex="" />
                                        </td>
                                        <td class="text-center">                                            
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-6">
                                <input type="hidden" name="group_price_id" value="<?php echo $edit_groupprice_info[0]->group_price_id; ?>">
                                <input type="submit" id="" class="btn btn-primary btn-large" name="add-user" value="Update" />
                            </div>
                        </div>
                        <?php echo form_close() ?>
                    </div>
...
// ============= its for row delete dynamically =========
    function deleteRow(t) {
        ...
        if (1 == a) {
            ...
            var common_pdt = 1;
            $("#normalinvoice > tbody > tr td select.common_pdt").each(function () {
                $(this).attr('id', 'product_id_' + common_pdt);
                $(this).attr('onchange', 'service_cals(this.value, ' + common_pdt + ');');
                common_pdt++;
            });
            var common_pdt_rate = 1;
            $("#normalinvoice > tbody > tr td input.common_pdt_rate").each(function () {
                $(this).attr('id', 'product_rate_' + common_pdt_rate);
                $(this).attr('onkeyup', 'quantity_calculate(' + common_pdt_rate + ');');
                $(this).attr('onchange', 'quantity_calculate(' + common_pdt_rate + ');');
                common_pdt_rate++;
            });
        }
        calculateSum();        
