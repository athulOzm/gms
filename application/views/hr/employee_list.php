...
//                                        echo '<pre>';                                        print_r($employee_list);echo '</pre>'; 
                                        ...
                                        <center>
                                            <?php echo form_open() ?>
                                            <?php if ($this->permission1->check_label('manage_employee')->update()->access()) { ?>                          
                                            ...
                                            <?php } ?>
                                            <?php if ($this->permission1->check_label('manage_employee')->delete()->access()) { ?>                                
                                                <a href="<?php echo base_url('Chrm/employee_delete/' . $employees['id']); ?>" class="DeleteEmployee btn btn-danger btn-sm" name="<?php echo $employees['id'] ?>" data-toggle="tooltip" data-placement="right" title="" data-original-title="<?php echo display('delete') ?> "><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                            <?php
                                            }
                                            if ($this->permission1->check_label('manage_employee')->read()->access()) {
                                                ?>
                                                <a href="<?php echo base_url('Chrm/employee_details/' . $employees['id']); ?>" class="btn btn-primary btn-sm"><i class="fa fa-user"></i></a>
                                                    <?php
                                                }
                                                echo form_close()
                                                ?>
