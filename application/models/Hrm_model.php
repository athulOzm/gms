...
//=========================== employeee data start ===========================
  ...
  $createdate=date('Y-m-d H:i:s');
   ...
   public function employee_editdata($id){
        $this->db->select('a.*, b.*, a.id as employee_row_id');
        $this->db->from('employee_history a');
     ...
     public function headcode(){
        $query=$this->db->query("SELECT MAX(HeadCode) as HeadCode FROM acc_coa WHERE HeadLevel='3' And HeadCode LIKE '5020401-%' ORDER BY row_id DESC LIMIT 1");
        return $query->row();
