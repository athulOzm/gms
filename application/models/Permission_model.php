...
class Permission_model extends CI_Model {
    ...
    public function module() {
        return $modules = $this->db->select('*')->from('sub_module')->where('status', 1)->group_by('module')->get()->result();
    }
...
// ============ its for get parent menu ===========
    public function get_parent_menu() {
        return $info = $this->db->select('*')->from('sub_module')->order_by('name', 'asc')->where('status', 1)->get()->result();
    }
...
//    ============ its for menu data edit mode =============
    ...
    public function menu_search($keyword) {
        ...
        $this->db->or_like('url', $keyword, 'both');
        $this->db->limit('100');
        $this->db->order_by('id', 'desc');
        $query = $this->db->get();
        return $query->result();
